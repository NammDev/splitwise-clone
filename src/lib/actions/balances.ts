...
export type Balances = Record<Participant['id'], { paid: number; paidFor: number; total: number }>
...
export function getBalances(
  ...
  for (const expense of expenses) {
    ...
    const totalPaidForShares = paidFors.reduce((sum, paidFor) => sum + paidFor.shares, 0)
    ...
    paidFors.forEach((paidFor, index) => {
      ...
      const dividedAmount = isLast ? remaining : (expense.amount * shares) / totalShares
  ...
  for (const participantId in balances) {
    ...
    balances[participantId].paidFor = Math.round(balances[participantId].paidFor) + 0
...
export function getPublicBalances(reimbursements: Reimbursement[]): Balances {
  ...
  reimbursements.forEach((reimbursement) => {
    ...
    if (!balances[reimbursement.to]) balances[reimbursement.to] = { paid: 0, paidFor: 0, total: 0 }
