...
export function getRecentGroups() {
  ...
  const groupsInStorageRaw = groupsInStorageJson ? JSON.parse(groupsInStorageJson) : []
...
export function deleteRecentGroup(group: RecentGroup) {
  const recentGroups = getRecentGroups()
  localStorage.setItem(STORAGE_KEY, JSON.stringify(recentGroups.filter((rg) => rg.id !== group.id)))
}

export function getStarredGroups() {
  const starredGroupsJson = localStorage.getItem(STARRED_GROUPS_STORAGE_KEY)
  const starredGroupsRaw = starredGroupsJson ? JSON.parse(starredGroupsJson) : []
...
export function starGroup(groupId: string) {
  ...
  localStorage.setItem(STARRED_GROUPS_STORAGE_KEY, JSON.stringify([...starredGroups, groupId]))
}
...
export function getArchivedGroups() {
  ...
  const archivedGroupsRaw = archivedGroupsJson ? JSON.parse(archivedGroupsJson) : []
...
export function archiveGroup(groupId: string) {
  ...
  localStorage.setItem(ARCHIVED_GROUPS_STORAGE_KEY, JSON.stringify([...archivedGroups, groupId]))
}
