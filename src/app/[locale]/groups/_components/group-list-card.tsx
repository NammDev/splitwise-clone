...
export function RecentGroupListCard({
  ...
  const details =
    state.status === 'complete' ? state.groupsDetails.find((d) => d.id === group.id) : null
  ...
  return (
    <li key={group.id}>
      <Button
        ...
        asChild
      >
        <div className='text-base' onClick={() => router.push(`/groups/${group.id}`)}>
          <div className='w-full flex flex-col gap-1'>
            <div className='text-base flex gap-2 justify-between'>
              <Link href={`/groups/${group.id}`} className='flex-1 overflow-hidden text-ellipsis'>
                {group.name}
              </Link>
              <span className='flex-shrink-0'>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button size='icon' variant='ghost' className='-my-3 -mr-3 -ml-1.5'>
