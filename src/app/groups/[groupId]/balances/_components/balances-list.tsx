import { Balances } from '@/lib/actions/balances'
...
export function BalancesList({ balances, participants, currency }: Props) {
  const maxBalance = Math.max(...Object.values(balances).map((b) => Math.abs(b.total)))
  ...
  return (
    <div className='text-sm'>
      {participants.map((participant) => {
        ...
        return (
          <div key={participant.id} className={cn('flex', isLeft || 'flex-row-reverse')}>
            <div className={cn('w-1/2 p-2', isLeft && 'text-right')}>{participant.name}</div>
            <div className={cn('w-1/2 relative', isLeft || 'text-right')}>
              <div className='absolute inset-0 p-2 z-20'>{formatCurrency(currency, balance)}</div>
              {balance !== 0 && (
                <div
                  className={cn(
