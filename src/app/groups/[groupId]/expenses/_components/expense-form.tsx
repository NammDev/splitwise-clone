...
import { ExpenseFormValues, SplittingOptions, expenseFormSchema } from '@/lib/schemas'
...
const getDefaultSplittingOptions = (group: Props['group']) => {
  ...
  const defaultSplitMode = localStorage.getItem(`${group.id}-defaultSplittingOptions`)
  if (defaultSplitMode === null) return defaultValue
  const parsedDefaultSplitMode = JSON.parse(defaultSplitMode) as SplittingOptions
  ...
  for (const parsedPaidFor of parsedDefaultSplitMode.paidFor) {
    if (!group.participants.some(({ id }) => id === parsedPaidFor.participant)) {
...
async function persistDefaultSplittingOptions(
  ...
  if (localStorage && expenseFormValues.saveDefaultSplittingOptions) {
    ...
    localStorage.setItem(`${groupId}-defaultSplittingOptions`, JSON.stringify(splittingOptions))
  }
}
  ...
  const form = useForm<ExpenseFormValues>({
    ...
    defaultValues: expense
      ...
      ? {
          ...
          amount: String((Number(searchParams.get('amount')) || 0) / 100) as unknown as number, // hack
      ...
      : {
          ...
          category: searchParams.get('categoryId') ? Number(searchParams.get('categoryId')) : 0, // category with Id 0 is General
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(submit)}>
        <Card>
          <CardHeader>
            <CardTitle>{isCreate ? <>Create expense</> : <>Edit expense</>}</CardTitle>
          </CardHeader>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='sm:order-1'>
                  ...
                  <FormDescription>Enter the date the expense was made.</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
                  ...
                  <div className='flex items-baseline gap-2'>
                    ...
                    <FormControl>
                      <Input
                        ...
                        step={0.01}
                        placeholder='0.00'
                        onChange={(event) => onChange(enforceCurrencyPattern(event.target.value))}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='sm:order-5'>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={getSelectedPayer(field)}>
                    <SelectTrigger>
                  ...
                  <FormDescription>Select the participant who paid the expense.</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
        ...
        <Card className='mt-4'>
          <CardHeader>
                ...
                onClick={() => {
                  ...
                  const newPaidFor = allSelected
                    ...
                    : group.participants.map((p) => ({
                        participant: p.id,
                        shares:
                          paidFor.find((pfor) => pfor.participant === p.id)?.shares ??
            ...
            <CardDescription>Select who the expense was paid for.</CardDescription>
          </CardHeader>
          <CardContent>
            <FormField
              control={form.control}
                        ...
                        return (
                          ...
                          >
                            <FormItem className='flex-1 flex flex-row items-start space-x-3 space-y-0'>
                              ...
                              <FormLabel className='text-sm font-normal flex-1'>{name}</FormLabel>
                            </FormItem>
                                      ...
                                      <div className='flex gap-1 items-center'>
                                        ...
                                        <FormControl>
                                          <Input
                                            ...
                                            step={
                                              form.getValues().splitMode === 'BY_AMOUNT' ? 0.01 : 1
                                            }
                                          />
                                        </FormControl>
            ...
            <Collapsible className='mt-5' defaultOpen={form.getValues().splitMode !== 'EVENLY'}>
                        ...
                        <FormControl>
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value='BY_SHARES'>Unevenly – By shares</SelectItem>
                              <SelectItem value='BY_PERCENTAGE'>
                                Unevenly – By percentage
                              </SelectItem>
                              <SelectItem value='BY_AMOUNT'>Unevenly – By amount</SelectItem>
                            </SelectContent>
                          </Select>
                        </FormControl>
                        <FormDescription>Select how to split the expense.</FormDescription>
                      </FormItem>
                    )}
                  />
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className='flex flex-row gap-2 items-center space-y-0 pt-2'>
                        ...
                        <div>
                          <FormLabel>Save as default splitting options</FormLabel>
                        </div>
                      </FormItem>
                    )}
                  />
        ...
        {runtimeFeatureFlags.enableExpenseDocuments && (
          <Card className='mt-4'>
            <CardHeader>
              ...
              <CardDescription>See and attach receipts to the expense.</CardDescription>
            </CardHeader>
            {/* <CardContent>
              <FormField
                control={form.control}
                name='documents'
                render={({ field }) => (
                  <ExpenseDocumentsInput documents={field.value} updateDocuments={field.onChange} />
                )}
              />
            </CardContent> */}
          </Card>
