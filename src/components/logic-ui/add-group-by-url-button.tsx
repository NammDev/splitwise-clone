...
import { saveRecentGroup } from '@/lib/actions/storage'
...
export function AddGroupByUrlButton({ reload }: Props) {
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    const [, groupId] = url.match(new RegExp(`${window.location.origin}/groups/([^/]+)`)) ?? []
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant='secondary'>Add by URL</Button>
      </PopoverTrigger>
      <PopoverContent
      ...
      >
        ...
        <p>If a group was shared with you, you can paste its URL here to add it to your list.</p>
